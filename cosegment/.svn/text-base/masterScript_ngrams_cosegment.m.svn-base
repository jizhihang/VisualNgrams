function masterScript_ngrams_cosegment(OBJINDS)
...
for objind = OBJINDS            % run either all concepts or a selected concept
            ...
            for ijk=2:5
                ...
                if ~exist(thisngram_uniqNsort_fname, 'file')
                    ...
                    if areAllFilesDone(ngramDatadir_obj_tmpdir, ngramGoogFileInfo(ijk), [], 1) ~= 0
                        ...
                        multimachine_grail_compiled(['objectNgramData_2012_mapper ' num2str(ijk) ' '  objname ' ' cleangramdir ' ' ngramDatadir_obj_tmpdir ' ' thisPOStag], ngramGoogFileInfo(ijk), ngramDatadir_obj_tmpdir, 25, [], 'all.q', 8, 0, OVERWRITE, 0);
    ...
    outfname_imgcl = [ngramImgClfrdir_obj '/' objname '_' num2str(ngramtype)  '_all_fastICorder.txt'];
    if ~exist(outfname_imgcl, 'file')
        ...
        outfname1 = [ngramImgClfrdir_obj '/' objname '_' num2str(ngramtype) '_all_fastICorder1.txt'];
        outfname2 = [ngramImgClfrdir_obj '/' objname '_' num2str(ngramtype) '_all_fastICorder2.txt'];
    ...
    ngramfname_dwld = [ngramImgClfrdir_obj '/' objname '_' num2str(ngramtype) '_all_fastICorder.txt'];
    ...
    outfname_imgcl_clus = [ngramImgClfrdir_obj '/' objname '_' num2str(ngramtype) '_all_fastClusters.txt'];
    supNgMatFname = [ngramImgClfrdir_obj '/' objname '_' num2str(ngramtype) '_all_fastClusters.mat'];
    if ~exist(outfname_imgcl_clus, 'file')
        disp('Cluster ngrams at image level');
        inpfname_imgcl1 = [ngramImgClfrdir_obj '/' objname '_' num2str(ngramtype) '_all_fastICorder1.txt'];
        getAdjacencyMatrix_imgCl(ngramImgClfrdir_obj, inpfname_imgcl1);         % select ngrams above cutoff and build edgemat
        
        outfname1 = [ngramImgClfrdir_obj '/' objname '_' num2str(ngramtype) '_all_fastClusters_info.txt'];
        accfname = [ngramImgClfrdir_obj '/' objname '_' num2str(ngramtype) '_all_fastICorder1.txt'];
    ...
    if ~exist(fname_imgcl_sprNg, 'file')
        ...
        inpfname = [ngramImgClfrdir_obj '/' objname '_' num2str(ngramtype) '_all_fastClusters.mat'];
    ...
    disp('%%% Testing with co-segmentation (on voc data)');
    modelname = 'joint';
    if exist([baseobjdir '/' ['baseobjectcategory_' objname] '_joint.mat'], 'file') && ~exist([baseobjdir '/' 'baseobjectcategory_' objname '_boxes_coseg_' testdatatype '_' testyear '_' modelname '.mat'], 'file') % test _joint model on test set (cluster version)
        resdir = [baseobjdir '/testFiles_coseg_' testyear '/'];
        num_ids = getNumImagesInDataset(baseobjdir, testyear, testdatatype, thisPOStag);
        if areAllFilesDone(resdir, num_ids, [], 1) ~= 0
            numjobsDetTest = min(200, areAllFilesDone(resdir, num_ids, [], 1));            
            %pascal_test_coseg_multi(baseobjdir, ['baseobjectcategory_' objname], testdatatype, testyear, testyear, modelname, thisPOStag);   % cluster version            
            compileCode_v2_depfun('pascal_test_coseg_multi', 1, 'linuxUpdateSystemNumThreadsToMax.sh');
            multimachine_grail_compiled(['pascal_test_coseg_multi ' baseobjdir ' ' ['baseobjectcategory_' objname] ' ' testdatatype ' ' testyear ' ' testyear ' ' modelname ' ' thisPOStag], num_ids, resdir, numjobsDetTest, [], 'all.q', 8, 0, OVERWRITE, 0);
            areAllFilesDone(resdir, num_ids);
        end
        pascal_test_coseg_reducer(baseobjdir, ['baseobjectcategory_' objname], testdatatype, testyear, testyear, modelname, thisPOStag);
        %copyfile([[baseobjdir '/display_' testdatatype '_' testyear '_' testyear] '/'  'jointMontage.jpg'], [wwwdispdir '/display_' testdatatype '_' testyear '_' testyear '_jointMontage.jpg']);        
        %displayDetection_perImage(['baseobjectcategory_' objname], testdatatype, cachedir, testyear, testyear, modelname);
    end
    myprintfn; myprintfn;
