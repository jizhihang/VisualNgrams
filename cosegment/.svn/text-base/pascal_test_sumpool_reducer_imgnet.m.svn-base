function pascal_test_sumpool_reducer_imgnet(cachedir, cls, testset, year, suffix, modelname, postag, testcls)
...
disp(['pascal_test_sumpool_reducer_imgnet(''' cachedir ''',''' cls ''',''' testset ''',''' year ''',''' suffix ''',''' modelname ''',''' postag ''',''' testcls ''');' ]);
...
try    
    ...
    for i = 1:num_ids    
        ...
        load([cachedir '/testFiles_' year '/' '/output_' ids{i} '.mat'], 'ds_save', 'bs_save', 'ds_sumsave');        
        ds_out{i} = ds_save; 
...
displayDetection_rankedMontages_v5_imgnet(cls, testset, cachedir, year, suffix, modelname, postag, testcls);
