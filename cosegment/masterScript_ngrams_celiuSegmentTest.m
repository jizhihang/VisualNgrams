function masterScript_ngrams_celiuSegmentTest(OBJINDS)
...
if ~exist('OBJINDS', 'var') || isempty(OBJINDS)
    ...
    OBJINDS = [1 7 13];
    %OBJINDS = [1 4 7 8 10 12 13 17 20 85];
end
...
for objind = OBJINDS            % run either all concepts or a selected concept
    ...
    diary([resultsdir '/ngram_models/' objname '/diaryOutput_celiuTest.txt']);        % save a log of the entire run for debugging/record purposes
    
    disp(['Doing base object category ' num2str(objind) '.' objname]);
            
    disp('%%% TESTING (on celiu data)');
