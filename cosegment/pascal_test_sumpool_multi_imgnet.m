function pascal_test_sumpool_multi_imgnet(cachedir, cls, testset, year, suffix, modelname, postag, testcls)
...
disp(['pascal_test_sumpool_multi_imgnet(''' cachedir ''',''' cls ''',''' testset ''',''' year ''',''' suffix ''',''' modelname ''',''' postag ''',''' testcls ''');' ]);
...
if strcmp(postag, 'NOUN')
    ids = textread(sprintf(VOCopts.seg.clsimgsetpath, testcls, testset), '%s');
end
...
for f = list_of_ims
    ...
    fname = [resdir '/output_' ids{f} '.mat'];
    try
        load(fname, 'ds_save', 'bs_save', 'ds_sumsave', 'ds_nonms', 'bs_nonms');
        ds_save;
    catch        
        ...
        if ~isempty(bs)
            ...
            ds_nonms = ds; bs_nonms = bs;
            % do percomponent nms
            ds_temp = []; bs_temp = [];
            for i=1:length(model.phrasenames)
                thiscompinds = find(bs_nonms(:,end-1) == i);
                I = nms(ds_nonms(thiscompinds, :), 0.5);
                ds_temp = [ds_temp; ds_nonms(thiscompinds(I), :)];
                bs_temp = [bs_temp; bs_nonms(thiscompinds(I), :)];
            end
            ds_nonms = ds_temp; bs_nonms = bs_temp;
        ...
        save(fname, 'ds_nonms', 'bs_nonms', '-append');
    end
