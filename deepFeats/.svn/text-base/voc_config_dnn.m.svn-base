function conf = voc_config_dnn(varargin)
...
conf = cv(conf, 'features.sbin', 8);    % keep sbin the same as I still want the cell & blocks to be same as HOG but just z-dim changes
conf = cv(conf, 'features.dim', 257);   % 256+1
conf = cv(conf, 'features.truncation_dim', 257);
conf = cv(conf, 'features.extra_octave', false);
