function model = train_dnn(model, pos, neg, warp, randneg, iter, ...
...
for t = 1:iter
    ...
    if ~cont || t > 1
        ...
        if stop_relabeling
            disp('stop_relabeling as hinge loss on fg examples doesnt reduce by much');
            break;
        end
    end
    ...
    for tneg = 1:negiter
        fprintf('%s %s iter: %d/%d, neg iter %d/%d\n', ...
            procid(), model.class, t, iter, tneg, negiter);
        ...
        save([conf.paths.model_dir '/intermediateModels/' model_name], 'model');
...
function [num_entries, num_examples] = poswarp(t, model, pos, tag)
% assumption: the model only has a single structure rule
% of the form Q -> F.

conf = voc_config_dnn();
savename = [conf.paths.model_dir '/intermediateModels/poswarp_' tag '.mat'];
if ~exist(savename, 'file')
    %warped = warppos(model, pos);
    warped_feat = warppos_dnn(model, pos);
    save(savename, 'warped_feat');
else
    load(savename, 'warped_feat');
end
...
for i = 1:numpos
    ...
    % get example    
    %feat = features(double(warped{i}), model.sbin);
    feat = warped_feat{i};    
    key = [i 0 0 0];
    bls = [obl; fbl] - 1;
...
for i = 1:batchsize:numpos
    ...
    parfor k = 1:thisbatchsize
        ...
        %pyra = featpyramid(im, model);  % get feature pyramid
        pyra = featpyramid_dnn(pos(j), model);
        boxes = pos(j).boxes;
        % disable cropping as features have already been extracted for full image, and cropping makes it tricky;
        % cropping is basically done here to make things faster and I am ok for things to be a bit slow (this step
        % is nonetheless not very time consuming)
...
for i = 1:batchsize:numneg
    ...
    parfor k = 1:thisbatchsize
        ...
        pyra = featpyramid_dnn(neg(j), model);
