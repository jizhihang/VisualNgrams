function [mimg, mlab, mimg_nolab] = getMontagesForModel_latent_wsup(inds, inds_past, inds_fut, posscores,...
...
[mimg, mlab, mimg_nolab] = deal(cell(numComps+1,1));    % +1 to accomodate 0 index    
for jj=1:numel(mimg)                        % initialize to dummy
    mimg{jj} = ones(10,10,3);
    mlab{jj} = ' ';
    mimg_nolab{jj} = ones(10,10,3);
end
...
for jj = 1:length(unids)    
        %disp([num2str(jj) ' ' num2str(unids(jj))]);
        myprintf(unids(jj));
        ...
        % added the following line but disabled it later as it increases
        % the overall time, rather doing this in a separate function
        %mimg_nolab{unids(jj)+1} = single(montage_list(allimgs, 2, [1 1 1], [1500 1500 3]));
end
myprintfn;
