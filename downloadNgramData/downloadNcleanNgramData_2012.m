...
if 1
    tmpdirname = '/zfs/isis1/zhenyang/Workspace/data/tmp/';
...
for f = list_of_ims    %0:numNgramFiles
    ...
    if ~exist(savename, 'file')
        ...
        %if ~exist([unzipfileprefix 'aa'], 'file')   % check if file has not already been unzipped
            disp(' unzipping');
            disp(['rm ' unzipfileprefix '*']);
            system(['rm ' unzipfileprefix '*']);
            disp(['gunzip -c ' zipfilename ' | split -C 1G - ' unzipfileprefix]);
        ...
        for i=1:numel(flist)    % for each of the split file
            ...
            disp(['cat ' unzipfilename ' | cut -f 1']);
            %[~, ngstrings] = system(['cat ' unzipfilename ' | cut -f 1'], '-echo');     % first field is string (ngram)
            system(['cat ' unzipfilename ' | cut -f 1 > tmp.txt']);
            ngstrings = textread('tmp.txt', '%s');
            disp(['cat ' unzipfilename ' | cut -f 3']);
            %[~, ngcnts] = system(['cat ' unzipfilename ' | cut -f 3'], '-echo');        % thrid field is the ngram count
            system(['cat ' unzipfilename ' | cut -f 3 > tmp.txt']);
            ngcnts = textread('tmp.txt', '%s');
