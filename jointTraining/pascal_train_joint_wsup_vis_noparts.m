function pascal_train_joint_wsup_vis_noparts(cls, objname, phrasenames, cachedir, year, fg_olap, borderoffset, jointCacheLimit, n_perngram, do_retraining)
...
disp(['pascal_train_joint_wsup_vis_noparts(''' cls ''',''' objname ''','' phrasenames '',''' cachedir ''',''' year ''',' num2str(fg_olap) ',' num2str(borderoffset) ',' num2str(jointCacheLimit) ',' num2str(n_perngram) ',' num2str(do_retraining) ')' ]);
...
minAPthresh_vis = conf.threshs.minAP_compThresh_vis;
...
try
    load([cachedir cls '_joint_visdata_noparts'], 'pos', 'impos', 'neg', 'models_all', 'model', ...
    ...
    for ii = 1:numel(phrasenames)
        ...
        [compaps_full, compaps, numTrngInst_mix] = deal(zeros(n_perngram,1));
        for ck=1:n_perngram
            compaps_full(ck) = roc{ck}.ap_full_new*100;
            compaps(ck) = roc{ck}.ap_new*100;
            numTrngInst_mix(ck) = numel(find(inds_mix == ck));
        end
        numTrngInst = model.stats.filter_usage;
        
        for j=1:n_perngram
            %if selcomps(j) == 1
            if (compaps_full(j) > minAPthresh_full && ceil(compaps(j)) >= minAPthresh_vis) && ...
                    (numTrngInst(j) >= minNumTrngInst || numTrngInst_mix(j) >= minNumTrngInst)
    ...
    [model.phrasenames, model.compSize] = getphraseInfo_modelmerged(models_all, [cachedir '/componentNamesWithIndices_vis.txt']);
    model.class = cls;
    ...
    save([cachedir cls '_joint_visdata_noparts.mat'], 'pos', 'impos', 'neg', 'models_all', 'model',...
        'inds_befjnt', 'posscores_befjnt', 'lbbox_befjnt', 'listOfSelNgramComps_globalIds',...
        'listOfSelNgramComps_accs');
    save([cachedir cls '_joint_vis_noparts'], 'model');
    %save([cachedir cls '_joint_vis_noparts'], 'posdata', 'poscell', '-append');    
end 
myprintfn;
