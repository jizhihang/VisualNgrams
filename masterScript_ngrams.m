function masterScript_ngrams(OBJVAL)
...
for objind = OBJINDS            % run either all concepts or a selected concept
    ...
    if areAllFilesDone_pascaldet(ngramModeldir_obj, ngramnames, [modelname '_goodInfo'], 1) == 0
        ...
        for ii = 1:numel(ngramnames)
            ...
            if ~exist([cachedir '/' ngramnames{ii} '_' modelname '_goodInfo.mat'], 'file')
                ...
                multimachine_grail_compiled(['pascal_train_wsup3 ' ngramnames{ii} ' ' num2str(dpm.numcomp) ' ' 'blah' ' ' cachedir  ' ' trainyear ' ' num2str(dpm.wsup_fg_olap) ' ' num2str(dpm.borderoffset) ' ' objname ' ' fname_imgcl_sprNg ' ' num2str(doparts)], 1, cachedir, 1, [], qnameval, 8, 0, OVERWRITE);
            end
        end
    ...
    if areAllFilesDone_pascaldet(ngramModeldir_obj, ngramnames, modsuffix, 1) == 0      % test selected _mix model components on their val set (for clustering)
        ...
        for ii = 1:numel(ngramnames)
            ...
            if  ~exist([cachedir '/' ngramnames{ii}  '_' modsuffix '.mat'], 'file')
                ...
                multimachine_grail_compiled(['pascal_test_sumpool_selectedComps ' cachedir ' ' ngramnames{ii} ' ' valdatatype2 ' ' trainyear ' ' trainyear ' ' modelname ' ' objname], 1, cachedir, 1, [], qnameval, 8, 0, OVERWRITE);
            end
        end
    ...
    if ~exist([baseobjdir '/' ['baseobjectcategory_' objname] '_joint_data.mat'], 'file')
        ...
        %multimachine_grail_compiled(['pascal_train_joint_wsup ' ['baseobjectcategory_' objname] ' ' objname ' ' fname_imgcl_sprNg ' ' baseobjdir ' ' trainyear ' ' num2str(dpm.wsup_fg_olap) ' ' num2str(dpm.borderoffset) ' ' num2str(dpm.jointCacheLimit) ' '  num2str(dpm.numcomp) ' '   num2str(do_retraining)], 1, baseobjdir, 1, [], qnameval, 8, 0, OVERWRITE, 0);
    end
    myprintfn; myprintfn;
    ...
    if exist([baseobjdir '/' ['baseobjectcategory_' objname] '_joint.mat'], 'file') && ~exist([baseobjdir '/' 'baseobjectcategory_' objname '_boxes_' testdatatype '_' testyear '_' modelname '.mat'], 'file') % test _joint model on test set (cluster version)
        ...
        if areAllFilesDone(resdir, num_ids, [], 1) ~= 0
            ...
            multimachine_grail_compiled(['pascal_test_sumpool_multi ' baseobjdir ' ' ['baseobjectcategory_' objname] ' ' testdatatype ' ' testyear ' ' testyear ' ' modelname ' ' thisPOStag], num_ids, resdir, numjobsDetTest, [], qnameval, 8, 0, OVERWRITE, 0);
