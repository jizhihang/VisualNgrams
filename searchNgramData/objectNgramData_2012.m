...
%minNgramCnt = 0;
%disp('minNgramCnt has been set to 0');
...
if ~exist(savename_final, 'file')    
    for ij = 1:numel(objname_syns)        
        ...
        savename = [outdir '/' strrep(thisobjname, ' ', '_')  '_' num2str(ngramtype) '_all_syn.txt'];
        savename1 = [outdir '/' strrep(thisobjname, ' ', '_')  '_' num2str(ngramtype) '_all_syn_wrkng.txt'];
        savename2 = [outdir '/' strrep(thisobjname, ' ', '_')  '_' num2str(ngramtype) '_all_syn_uniquedNsort.txt'];        
        savename3 = [outdir '/' strrep(thisobjname, ' ', '_')  '_' num2str(ngramtype) '_all_syn_uniquedNsort_rewrite.txt'];
        ...
        parfor f=1:numNgramFiles
            myprintf(f,100);
            savename_tmp = [tmpdir '/' strrep(thisobjname, ' ', '_')  '_' num2str(ngramtype) '_tmp_' num2str(f) '.txt'];
        ...
        elseif ngramtype == 2 || ngramtype == 3 || ngramtype == 4 || ngramtype == 5  
            filenameWithPath = which('rewriteNgramdata_2012_2345.sh');   %/projects/grail/santosh/objectNgrams/code/ngram/rewriteNgramdata_2012_2345.sh'
            system([filenameWithPath ' ' ...
                savename2 ' ' savename3]);
        end
    end
    ...
    for ij = 1:numel(objname_syns)           % cant parfor this as appending to file
        myprintf(ij);
        thisobjname = objname_syns{ij};
        savename_tmp = [outdir '/' strrep(thisobjname, ' ', '_')  '_' num2str(ngramtype) '_all_syn_uniquedNsort_rewrite.txt'];
