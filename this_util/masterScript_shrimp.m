function masterScript_shrimp(OBJINDS)
...
if DO_NGRAM_CORPUS_DWNLD            % download (from https://books.google.com/ngrams/datasets) & compress ngram data corpus
    ...
    if ngramtype_global == 0
        ...
        %multimachine_grail_compiled(['downloadNcleanNgramData_2012 ' num2str(ngramtype) ' ' cleangramdir], ngramGoogFileInfo(ngramtype), rawngramdir, ngramGoogFileInfo(ngramtype), [], 'notcuda.q', 16, 0, OVERWRITE);
    elseif ngramtype_global == 2345
        for ijk=2:5
            ...
            if areAllFilesDone(cleangramdir, ngramGoogFileInfo(ijk), [], 1) ~= 0
                ...
                multimachine_grail_compiled(['downloadNcleanNgramData_2012 ' num2str(ijk) ' ' cleangramdir], ngramGoogFileInfo(ijk), cleangramdir, numjobsDwldNcln, [], 'notcuda.q', 8, 0, OVERWRITE, 0);
...
for objind = OBJINDS            % run either all concepts or a selected concept
            ...
            for ijk=2:5
                ...
                if ~exist(thisngram_uniqNsort_fname, 'file')
                    ...
                    if areAllFilesDone(ngramDatadir_obj_tmpdir, ngramGoogFileInfo(ijk), [], 1) ~= 0
                        ...
                        multimachine_grail_compiled(['objectNgramData_2012_mapper ' num2str(ijk) ' '  objname ' ' cleangramdir ' ' ngramDatadir_obj_tmpdir ' ' thisPOStag], ngramGoogFileInfo(ijk), ngramDatadir_obj_tmpdir, 25, [], 'notcuda.q', 8, 0, OVERWRITE, 0);
    ...
    if ~exist(outfname_imgcl, 'file')
        ...
        if areAllFilesDone(ngramImgClfrdir_obj, numngrams, [], 1) ~= 0
            ...
            if numjobsImgCls < 10   % if only a few ngrams, then run locally (otherwise on cluster)
                ...
                multimachine_grail_compiled(['pascal_img_trainNtestNeval_fast ' ngramImgClfrdir_obj ' ' inpfname_imgcl ' ' trainyear ' ' objname ' ' imgannodir], numngrams, ngramImgClfrdir_obj, numjobsImgCls, [], 'notcuda.q', 8, 0, OVERWRITE, 0);
            end
    ...
    if areAllFilesDone(rawgoogimgdir_obj, numngrams, [], 1) ~= 0        
        ...
        if numjobsDld <= 10
            ...
            compileCode_v2_depfun('downloadGoogImgs', 1, 'samGoogDownload.sh', 'samYahooDownload.sh', 'googleimages_dsk.py');            
            multimachine_grail_compiled(['downloadGoogImgs ' ngramfname_dwld ' ' rawgoogimgdir_obj], numngrams, rawgoogimgdir_obj, numjobsDld, [], 'notcuda.q', 8, 0, OVERWRITE, 0);
        end
        areAllFilesDone(rawgoogimgdir_obj, numngrams);
    end
    myprintfn; myprintfn;
         
    disp('disabled merging, instead copied _all_fastICorder.txt to _all_fastClusters_super.txt'); 
    %{
    ...
    if ~exist([ngramDupdir_obj '/resultsHashing_' dtype '/' dtype '_' objname '.txt'], 'file')
        ...
        if areAllFilesDone(cachedir, numimgs, [], 1) ~= 0
            ...
            %multimachine_grail_compiled(['findNearDuplicates_hashing ' cachedir ' ' dtype], numimgs, cachedir, 100, [], 'notcuda.q', 8, 0, OVERWRITE, 0);
    ...
    disp('disabled duplicate check as you donot want to merge even at component level for shrimp'); 
    %{
    ...
    if ~exist([imgsetdir '/baseobjectcategory_' objname '_' dtype '_withDups.txt'], 'file')
        ...
        if areAllFilesDone(cachedir, numimgs, [], 1) ~= 0
            ...
            multimachine_grail_compiled(['findNearDuplicates_hashing ' cachedir ' ' dtype], numimgs, cachedir, numjobsImgDups, [], 'notcuda.q', 8, 0, OVERWRITE, 0);
            areAllFilesDone(cachedir, numimgs);
            %findNearDuplicates_hashing(cachedir, dtype);            
        end
    ...
    if areAllFilesDone_pascaldet(ngramModeldir_obj, ngramnames, [modelname '_goodInfo'], 1) == 0
        ...
        for ii = 1:numel(ngramnames)
            ...
            if ~exist([cachedir '/' ngramnames{ii} '_' modelname '_goodInfo.mat'], 'file')
                ...
                multimachine_grail_compiled(['pascal_train_wsup3 ' ngramnames{ii} ' ' num2str(dpm.numcomp) ' ' 'blah' ' ' cachedir  ' ' trainyear ' ' num2str(dpm.wsup_fg_olap) ' ' num2str(dpm.borderoffset) ' ' objname ' ' fname_imgcl_sprNg ' ' num2str(doparts)], 1, cachedir, 1, [], 'notcuda.q', 8, 0, OVERWRITE);
            end
        end
    ...
    if areAllFilesDone_pascaldet(ngramModeldir_obj, ngramnames, modsuffix, 1) == 0      % test selected _mix model components on their val set (for clustering)
        ...
        for ii = 1:numel(ngramnames)
            ...
            if  ~exist([cachedir '/' ngramnames{ii}  '_' modsuffix '.mat'], 'file')
                ...
                multimachine_grail_compiled(['pascal_test_sumpool_selectedComps ' cachedir ' ' ngramnames{ii} ' ' valdatatype2 ' ' trainyear ' ' trainyear ' ' modelname ' ' objname], 1, cachedir, 1, [], 'notcuda.q', 8, 0, OVERWRITE);
            end
        end
    ...
    disp('%%% CREATE JOINT MODEL (without parts) ACROSS ALL NGRAM MODELS (merge compoments)');  % added as this info may be needed even when parts are not trained
    %ngramnames = getNgramNamesForObject_new(objname, fname_imgcl_sprNg);  % get list of all ngram names
    ...
    disp('%%% CREATE JOINT MODEL FOR VIS (without parts) ACROSS ALL NGRAM MODELS (merge compoments)');  % added as this info may be needed even when parts are not trained
    %ngramnames = getNgramNamesForObject_new(objname, fname_imgcl_sprNg);  % get list of all ngram names
    if ~exist([baseobjdir '/' ['baseobjectcategory_' objname] '_joint_visdata_noparts.mat'], 'file')
        do_retraining = 0;        
        pascal_train_joint_wsup_vis_noparts(['baseobjectcategory_' objname], objname, ngramnames, baseobjdir, trainyear, dpm.wsup_fg_olap, dpm.borderoffset, dpm.jointCacheLimit, dpm.numcomp, do_retraining);
    end
    myprintfn; myprintfn;
    ...
    if ~exist([baseobjdir '/' ['baseobjectcategory_' objname] '_joint_data.mat'], 'file')
        ...
        %multimachine_grail_compiled(['pascal_train_joint_wsup ' ['baseobjectcategory_' objname] ' ' objname ' ' fname_imgcl_sprNg ' ' baseobjdir ' ' trainyear ' ' num2str(dpm.wsup_fg_olap) ' ' num2str(dpm.borderoffset) ' ' num2str(dpm.jointCacheLimit) ' '  num2str(dpm.numcomp) ' '   num2str(do_retraining)], 1, baseobjdir, 1, [], 'notcuda.q', 8, 0, OVERWRITE, 0);
    end
    myprintfn; myprintfn;
    ...
    datayear = testyear; testset_spl = testdatatype; VOCinit;
    if 1 %~(  (str2num(testyear) <= 2007) || isAnnotationExists(VOCopts, thisPOStag)  )
        disp('no annotation exists, so no model testing; continuing with next concept');
        continue;
    end 
    ...
    if exist([baseobjdir '/' ['baseobjectcategory_' objname] '_joint.mat'], 'file') && ~exist([baseobjdir '/' 'baseobjectcategory_' objname '_boxes_' testdatatype '_' testyear '_' modelname '.mat'], 'file') % test _joint model on test set (cluster version)
        ...
        if areAllFilesDone(resdir, num_ids, [], 1) ~= 0
            ...
            multimachine_grail_compiled(['pascal_test_sumpool_multi ' baseobjdir ' ' ['baseobjectcategory_' objname] ' ' testdatatype ' ' testyear ' ' testyear ' ' modelname ' ' thisPOStag], num_ids, resdir, numjobsDetTest, [], 'notcuda.q', 8, 0, OVERWRITE, 0);            
