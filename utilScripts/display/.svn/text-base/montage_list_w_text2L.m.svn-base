function montage_image_text = montage_list_w_text2L(scene_match_files, ...
...
whitespace1 = 15;
whitespace = 2*whitespace1; %half of the number of pixels of white space to put between everything

% udpated 2Apr12
montage_dimensions_min = [montage_tiles_y * 220, montage_tiles_x * 220, 3];
if ~exist('montage_dimensions', 'var') || isempty(montage_dimensions)
    montage_dimensions = montage_dimensions_min;
else
    if montage_dimensions(1) < montage_dimensions_min(1) || montage_dimensions(2) < montage_dimensions_min(2)
        montage_dimensions = montage_dimensions_min;
    end
end
...
for i = 1:num_images
    ...
    t_handle = text(start_x + 1, end_y - whitespace/2 + 1, scene_match_labels{i,1});
    set(t_handle, 'FontSize', tfontsize);
    t_handle = text(start_x + 1, end_y + whitespace/2 + 1, scene_match_labels{i,2});
    set(t_handle, 'FontSize', tfontsize);
end
